...
run_test() {
	...
	# use debug builds so we can make requests to non-TLS fake-server
...
other_commands() {
	run_test "--list-themes" "bws --list-themes | grep -q 'Dracula'" # assumes Dracula theme is always available in bat
}

main() {
	pushd "${REPO_ROOT}" >/dev/null || exit 1

	echo "Building bws with debug profile..."
	cargo build --bin bws --quiet
	echo
